...
export function mixtape(id: number, func: "add" | "drop") {
  if (func === "add") {
...
const handlePauseSong = (ev: MouseEvent) => {
  ...
  const singerId: string | null = (ev.target as HTMLDivElement).id;
  if (
    songId &&
    (ev.target as HTMLDivElement).classList.contains("singer") &&
    throttle === 0 &&
    singerId
  ) {
    console.log("pausing");
