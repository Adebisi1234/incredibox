  ...
  const animation = () => {
    ...
    requestAnimationFrame(() => {
      if (!global.timeouts[songId].paused) {
        ...
        ctx?.drawImage(
          img,
          animeFrame.arrayFrame[i][0],
          animeFrame.arrayFrame[i][1],
          imgWidth,
          animeFrame.headHeight,
          (canvas.width / 8) * (+singerId - 1) + animeFrame.arrayFrame[i][2],
          (canvas.width / 8) * (+singerId - 1) + animeFrame.arrayFrame[i][3],
          canvas.width / 8,
          animeFrame.headHeight * canvasToIntrinsicRatio
        );
      }
